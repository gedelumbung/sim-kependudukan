...
class CrudCode extends CCodeModel
{
	...
	public function generateInputField($modelClass,$column)
	{
		...
		else
		{
			...
			if($column->type!=='string' || $column->size===null)
				return "CHtml::{$inputField}(\$model,'{$column->name}',array('class'=>'input-block-level'))";
			else
			{
				if(($size=$maxLength=$column->size)>60)
					$size=60;
				return "CHtml::{$inputField}(\$model,'{$column->name}',array('size'=>$size,'maxlength'=>$maxLength,'class'=>'input-block-level'))";
			}
		}
	}
	...
	public function generateActiveField($modelClass,$column)
	{
		...
		elseif(stripos($column->dbType,'text')!==false)
			return "\$form->textArea(\$model,'{$column->name}',array('rows'=>6, 'cols'=>50, 'class'=>'input-block-level'))";
		else
		{
			if(preg_match('/^(password|pass|passwd|passcode)$/i',$column->name))
			...
			if($column->type!=='string' || $column->size===null)
				return "\$form->{$inputField}(\$model,'{$column->name}', array('class'=>'input-block-level'))";
			else
			{
				if(($size=$maxLength=$column->size)>60)
					$size=60;
				return "\$form->{$inputField}(\$model,'{$column->name}',array('size'=>$size,'maxlength'=>$maxLength, 'class'=>'input-block-level'))";
			}
		}
	}
